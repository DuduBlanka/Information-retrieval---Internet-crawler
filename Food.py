...
# Retrieve the HTML of the product page
 ...
 for label in labels:
         unit = label['data-weight-unit']
         if unit == '':
            grab_from_span = True
            break
         weight = label['data-weight']
         weight_and_unit = f"{weight} {unit}"
         
         if (name,weight_and_unit) not in weight_List:
            weight_List.append((name,weight_and_unit))
 if weight_and_unit is None or grab_from_span == True:
     spans = product_soup.find_all('span', class_='prWeight')
     ...
     for span in span_elements:
        ...
        if span.next_sibling:
            if span.next_sibling.string:
                ...
                if weight_units == 'מ"ל' or weight_units == 'ק"ג' :
...
with open('Weight.csv', 'w', encoding='utf-8-sig', newline='') as csv_file:
    # Create a CSV writer object
    csv_writer = csv.writer(csv_file)

    # Write the header row
    csv_writer.writerow(['Product Name', 'Product_Weight'])
